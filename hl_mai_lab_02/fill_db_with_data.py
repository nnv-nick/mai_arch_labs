...
for cur_id in range(5):
    for cur_user_id in range(5):
        ...
        subprocess.run(f'curl -X POST -H "Content-Type: application/json" -d \'{json.dumps(route_json)}\' http://localhost:8080/route', shell=True)

trip_json = {"id": 1, "route_id": 3, "user_ids": [0, 3]}
subprocess.run(f'curl -X POST -H "Content-Type: application/json" -d \'{json.dumps(trip_json)}\' http://localhost:8080/trip', shell=True)
