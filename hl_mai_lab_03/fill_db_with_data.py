...
jwt_token_header = '-H "Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpYXQiOjE3MTY2NjkzNzEuNTgwNzk4LCJpZCI6MSwibG9naW4iOiJ0am9pb253Iiwic3ViIjoibG9naW4ifQ.3xWN-5tAALurWdohYc_M3GMBJaxWanIOL1ilD1Cu_Ao"'
...
for cur_id in range(5):
    for cur_user_id in range(5):
        ...
        subprocess.run(f'curl -X POST {jwt_token_header} -H "Content-Type: application/json" -d \'{json.dumps(route_json)}\' http://localhost:8080/route', shell=True)

trip_json = {"id": 1, "route_id": 3, "user_ids": [0, 3]}
subprocess.run(f'curl -X POST {jwt_token_header} -H "Content-Type: application/json" -d \'{json.dumps(trip_json)}\' http://localhost:8080/trip', shell=True)
